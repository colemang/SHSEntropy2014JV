...
#include <math.h>
    ...
    bool EntropyDrive::Initialize () {
    	// create QuadratureEncoders to read the shaft rotation
        mRightEncoder = new Encoder(1, 2, true, Encoder::k4X);
        mLeftEncoder = new Encoder(3, 4, false, Encoder::k4X);
	...
	void EntropyDrive::ResetCounters()
	{
        mRightEncoder->Reset();
        mLeftEncoder->Reset();
        mRightEncoder->Start();
	...
	bool EntropyDrive::DriveRobot(float MoveValue, float RotateValue, DriverStationLCD* ds, bool CorrectMode){
		// make sure our inputs are clean
		MoveValue=Limit(MoveValue);
		RotateValue=Limit(RotateValue*0.5);
		
		// read the encoders
		int OldLeft = mLeftQuad;
	    ...
	    if (CorrectMode)
	    {
	    	...
	    	if (fabs (mRightMotor) > 0.50 && F1 != 0.0) RightCorrect = F2 / F1;
	    }
		...
		float LeftMotors = Limit(coeff * mLeftMotor *LeftCorrect + left *(1-coeff));  			
		float RightMotors = Limit(coeff * mRightMotor *RightCorrect + right *(1-coeff));
		
		//Command motors
		wpiDrive->SetLeftRightMotorOutputs( LeftMotors, RightMotors );
		
		// save the current driver value for use next time
		mRightMotor=RightMotors;
		mLeftMotor=LeftMotors;
		
		// debug output
		ds->PrintfLine(DriverStationLCD::kUser_Line4, "RM: %f", mRightMotor);
		ds->PrintfLine(DriverStationLCD::kUser_Line5, "LM: %f", mLeftMotor);		
		ds->UpdateLCD();
		return true;
	}
	...
	float EntropyDrive::right_scale(float rotateValue, float moveValue, DriveMode mode)
	{
		...
		float temp_drive = 0;
		int x_idx = 0;
		float absRotate = rotateValue;//experiment
		return -rotateValue;
		if(mode == Radius)
		{
			absRotate = fabs(rotateValue);
