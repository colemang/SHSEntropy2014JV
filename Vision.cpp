...
double Vision::TakeDistancePicture( DriverStationLCD* ds, int bottomH, int topH, int bottomS, int topS, int bottomV, int topV)
{
	double distance = 0;
	Scores *scores;
	TargetReport target;
...
//	Threshold threshold(105, 137, 230, 255, 133, 183);	//HSV threshold criteria, ranges are in that order ie. Hue is 60-100
	Threshold threshold(bottomH, topH, bottomS, topS, bottomV, topV);	//HSV threshold criteria, ranges are in that order ie. Hue is 60-100
	ParticleFilterCriteria2 criteria[] = {
			{IMAQ_MT_AREA, AREA_MINIMUM, 65535, false, false}
	};												//Particle filter criteria, used to filter out small particles
	AxisCamera &camera = AxisCamera::GetInstance( "10.1.38.11");	//To use the Axis camera uncomment this line
	...
	ds->PrintfLine(DriverStationLCD::kUser_Line6, "Vision Processing (step 1)...");
	printf("Vision Processing (step 1)...");
	
	image->Write("/initial.bmp");
	ds->PrintfLine(DriverStationLCD::kUser_Line6, "Vision Processing (step 2)...");
	printf("Vision Processing (step 2)...");
	BinaryImage *thresholdImage = image->ThresholdHSV(threshold);	// get just the green target pixels
	ds->PrintfLine(DriverStationLCD::kUser_Line6, "Vision Processing (step 3)...");
	printf("Vision Processing (step 3)...");
	thresholdImage->Write("/threshold.bmp");
	BinaryImage *filteredImage = thresholdImage->ParticleFilter(criteria, 1);	//Remove small particles
	ds->PrintfLine(DriverStationLCD::kUser_Line6, "Vision Processing (step 4)...");
	printf("Vision Processing (step 4)...");
	filteredImage->Write("/Filtered.bmp");

	vector<ParticleAnalysisReport> *reports = filteredImage->GetOrderedParticleAnalysisReports();  //get a particle analysis report for each particle
	ds->PrintfLine(DriverStationLCD::kUser_Line6, "Vision Processing (step 5)...");
	printf("Vision Processing (step 5)...");
	...
	ds->PrintfLine(DriverStationLCD::kUser_Line6, "Vision Processing (done)...");
	printf( "Finished Vision Code");
